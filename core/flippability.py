...
class FlippabilityCalculator:
    ...
    def _compute_score1(self, block_3x3):
        """
        计算3x3块的可翻转性分数
        ...
        Step 2: 详细评分
            2.1 初始化基础分 SB = 0.5
            2.2 评估平滑度
                - 规则1: 如果 T_H = 0 或 T_V = 0 → 减 w1 (0.25)
                - 规则2: 如果 min(T_H, T_V, T_D1, T_D2) < T1 (2) → 减 w2 (0.125)
            2.3 评估翻转影响
                - 计算翻转前后总转换数 T_total 和 T'_total
                - 规则3: 如果 T'_total == T_total → 加 w3 (0.125)
                - 规则4: 如果 T'_total > T_total → 减 w3 (0.125)
            2.4 评估连通性
                - 规则5: 如果连通分量数改变 → 减 w4 (0.125)
            2.5 最终分数钳位到 [0, 1]
        ...
        # 2.1 初始化基础分
        base_score = self.SB  # 0.5

        # 计算原始块的转换数
        T_H, T_V, T_D1, T_D2 = self._compute_transitions(block)
        total_transitions = T_H + T_V + T_D1 + T_D2
        ...
        # 规则1: 如果 T_H = 0 或 T_V = 0，减 w1
        if T_H == 0 or T_V == 0:
            base_score -= self.w1  # 减0.25

        # 规则2: 如果任一方向转换数小于 T1 (2)，减 w2
        min_transitions = min(T_H, T_V, T_D1, T_D2)
        if min_transitions < self.T1:  # T1 = 2
            base_score -= self.w2  # 减0.125
        ...
        total_transitions_f = T_H_f + T_V_f + T_D1_f + T_D2_f

        # 规则3: 如果总转换数不变，加 w3
        if total_transitions_f == total_transitions:
            base_score += self.w3  # 加0.125
        # 规则4: 如果总转换数增加，减 w3
        elif total_transitions_f > total_transitions:
            base_score -= self.w3  # 减0.125
        # 注意：如果总转换数减少，不扣分也不加分（论文未提及）
